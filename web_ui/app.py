...
def load_root_metrics():
    """Load model evaluation metrics from root metrics.json if available."""
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(script_dir)
        ...
        if os.path.exists(metrics_path):
            ...
            for item in model_results:
                ...
                key = name_map.get(m_name)
                if key and key in model_metadata:
                    model_metadata[key].update({
                        'accuracy': float(item.get('accuracy', model_metadata[key]['accuracy'])),
                        'precision': float(item.get('precision', model_metadata[key]['precision'])),
                        'recall': float(item.get('recall', model_metadata[key]['recall'])),
                        'f1': float(item.get('f1_score', item.get('f1', model_metadata[key]['f1']))),
                    })
            logger.info("✓ Loaded independent evaluation metrics from root metrics.json")
    except Exception as e:
        logger.error(f"Error loading root metrics.json: {e}")
...
class LSTMModel:
    ...
    def load(self):
        ...
        try:
            ...
            logger.info(f"  Accuracy: {model_metadata['lstm']['accuracy']:.4f}")
            return True
        except Exception as e:
