...
def load_dataset(filepath='../../dataset/final_spam_ham_dataset.csv'):
    """
...
def train_model(model, X_train, y_train, X_val, y_val):
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            filepath='model_files/best_model_checkpoint.h5',
            monitor='val_accuracy',
            save_best_only=True,
            verbose=1,
            mode='max'
...
def save_artifacts(model, tokenizer, label_encoder, metrics, max_length):
    ...
    # Save model
    model.save('model_files/lstm_spam_model.h5')
    print(" Model saved as 'model_files/lstm_spam_model.h5'")
    
    # Save tokenizer
    with open('model_files/tokenizer.pkl', 'wb') as f:
        pickle.dump(tokenizer, f)
    print(" Tokenizer saved as 'model_files/tokenizer.pkl'")
    
    # Save label encoder
    with open('model_files/label_encoder.pkl', 'wb') as f:
        pickle.dump(label_encoder, f)
    print(" Label encoder saved as 'model_files/label_encoder.pkl'")
    ...
    with open('model_files/model_config.pkl', 'wb') as f:
        pickle.dump(config, f)
    print(" Model configuration saved as 'model_files/model_config.pkl'")
    
    print("\nAll artifacts saved successfully!")
    print("Files created:")
    print("  - model_files/lstm_spam_model.h5 (trained model)")
    print("  - model_files/tokenizer.pkl (text tokenizer)")
    print("  - model_files/label_encoder.pkl (label encoder)")
    print("  - model_files/model_config.pkl (configuration)")
...
def check_existing_artifacts():
    ...
    """
    required_files = [
        'model_files/lstm_spam_model.h5',
        'model_files/tokenizer.pkl', 
        'model_files/label_encoder.pkl',
        'model_files/model_config.pkl'
    ]
